package jp.artan.dmlreloaded.neoforge.item.entity;
        ...
        if(isInWater()) {
            ...
            if(!level().isClientSide) {
                if (progress >= 23) {
                    ...
                    ItemEntity newItem = new ItemEntity(level(), position().x, position().y + 0.6D, position().z, new ItemStack(DMLItems.GLITCH_INGOT.get(), 1));
