            ...
            if(itemStack.getItem() instanceof ItemGlitchHeart && rand.nextInt(0, 10) <= 3) {
                if(player.getLevel().getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) {
                    ...
                    if(!player.getLevel().isClientSide) {
                        ...
                        ItemEntity drop = new ItemEntity(player.getLevel(), dropX, dropY, dropZ,
                                new ItemStack(Registry.ITEM.get(DeepMobLearningReloadedMod.getResource("glitch_fragment")), 3),
